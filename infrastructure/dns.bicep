...
@description('Custom RBAC role to allow management of A records in a DNS Zone')
resource dnsAContributrorRole 'Microsoft.Authorization/roleDefinitions@2022-04-01' = {
  name: customRoleName
  properties: {
    roleName: 'DNS A Contributor - ${uniqueSuffix}'
    description: 'Manage DNS A records only.'
    type: 'customRole'
    permissions: [
      {
...
@description('Allows Function App Managed Idetity to manage DNS A records')
resource funcMIDnsRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(functionAppId, dnsZone.id, dnsAContributrorRole.id)
  scope: dnsZone
  properties: {
    roleDefinitionId: dnsAContributrorRole.id
